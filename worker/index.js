...
Units: precipitation mm, snowfall cm, temperature anomaly in C. trajectory must contain exactly ${MONTHS.join(", ")} in order. seasonPhases must contain exactly early, mid, late in that order. Phase precip/snow are period totals; phase predictedTemp is the period mean anomaly. details must contain exactly ${REGIONS.join(", ")} in order with a non-empty risks string for each region. Statewide values are area-weighted seasonal totals. Regional normal must be a positive climatological precipitation baseline.`;

      const response = await fetch("https://api.mistral.ai/v1/chat/completions", {
        method: "POST",
        headers: { "authorization": `Bearer ${env.MISTRAL_API_KEY}`, "content-type": "application/json" },
        body: JSON.stringify({ model: env.MISTRAL_MODEL || "mistral-small-latest", temperature: 0.2, max_tokens: 4000, response_format: { type: "json_object" }, messages: [{ role: "system", content: "You are a California hydroclimate forecasting assistant. Return valid JSON only." }, { role: "user", content: prompt }] }),
      });
